...
int main (int argc, const char * argv[])
{
    ...
    // insert code here...
    std::cout << "usage: " << argv[0] << " hostsPerRack ipBase startId endId flowFile scaleFactorSize scaleFactorTime bitRateSingleHost generateInRackTraffic latency\n";
    ...
    while(!infile.eof()) {
        ...
        if(id >= startid && id <= endid) {
			...
			cout << "flow from " << f.fromIP << " to " << f.toIP  << endl;
